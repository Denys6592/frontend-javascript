...
const Main = () => {
  ...
  const handleJobPosting = async () => {
    ...
    await axios
      .post(`${apiUrl}/api/projects/new`, project, {
        headers: {
  ...
  const generateJobDescription = async () => {
    try {
      ...
      //////outside api call
      const response = await axios.post("http://155.130.70.168:8001/generate_job_posting", formData, {
        headers: {
      ...
      case 5:
        if (project.type === "hourly") {
          if (!project.minHourlyRate.trim() || !project.maxHourlyRate.trim()) {
            errorMessages.push("Both minimum and maximum hourly rates are required.");
          } else if (
            Number(project.minHourlyRate) > Number(project.maxHourlyRate)
          ) {
            errorMessages.push("Minimum hourly rate cannot be greater than maximum hourly rate.");
          }
      ...
      component: (
        <div className="flex flex-col gap-10 text-black py-5">
          <div className="flex flex-col gap-2.5" ref={dropdownRef}>
            ...
            <div
              className="flex items-center gap-2 w-full border border-black rounded-lg p-2.5 outline-none hover:border-blue400 hover:ring focus-within:border-blue400 focus-within:ring">
            ...
            {filteredSkills.length > 0 && (
              <ul
                className="absolute z-10 mt-[85px] bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
  ...
  return (
    <div
      className="flex flex-col gap-[50px] w-[1200px] h-[720px] p-[50px] bg-white rounded-[10px]">
