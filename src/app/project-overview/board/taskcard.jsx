...
import { formatDate, apiUrl } from "@/utils/constant";
...
const TaskCard = ({
  ...
  const handleTitleBlur = () => {
    ...
    if (editedTitle !== name) {
      axios
        .put(`${apiUrl}/api/tmProjects/task/${_id}`, { name: editedTitle }, {
        headers: {
  ...
  const confirmAction = () => {
    ...
    if (actionType === "delete") {
      ...
      axios
        ...
        .then(() => {
          ...
          const updatedTasks = (tmTasks ?? []).filter((task) => task._id !== _id);
    ...
    if (actionType === "archive") {
      ...
      axios
        .put(`${apiUrl}/api/tmProjects/task/${_id}`, { isActive: false }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
    ...
    if (actionType === "duplicate") {
      ...
      axios
        .post(`${apiUrl}/api/tmProjects/task/duplicate`, { id: _id }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  return (
    <div className="flex flex-col bg-white rounded-2xl p-4 gap-3 shadow-sm">
      ...
      <div className="">
        <div
          className={`inline-block px-3 py-1 text-xs font-medium rounded-full ${getPriorityColor(priority ?? "")}`}>
          {priority}
        </div>
      </div>
      ...
      {confirmModalOpen && (
        <div
          className="fixed inset-0 bg-[#17171819] flex justify-center items-center z-50">
          <div
            className="flex flex-col max-w-[400px] bg-white p-6 rounded-xl shadow-lg gap-4">
            {actionType === "archive" ? (
              <svg
                width="56"
                height="56"
