  ...
  const onDragEnd = (result) => {
    ...
    if (sourceColumnId !== destColumnId) {
      ...
      axios
        ...
        .then(() => {
          console.log(`Task ${movedTask._id} status updated to ${movedTask.status}`);
        })
        .catch((err) => {
          console.log(err);
        });
        ...
        {Object.entries(columns).map(([columnId, column]) => (
          <div key={columnId} className="flex flex-col gap-4">
            <div className="flex items-center font-normal text-md gap-2.5">
              <span className={`w-3 h-3 rounded-full ${columnColors[columnId]}`}></span>
                        ...
                        <Draggable key={task._id} draggableId={task._id ?? "1"} index={index}>
                          {(provided) => (
                            <div
                              ...
                              <TaskCard {...task} tmTasks={tmTasks || []} setTmTasks={setTmTasks} />
                            </div>
                          )}
                        </Draggable>
                      ))
