...
import { apiUrl, getCountryName } from "@/utils/constant";
...
const ProposalCard = ({
  ...
  const handleViewProposal = () => {
    ...
    axios
      .get(`${apiUrl}/api/users/user/full`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        params: { id: proposal.user._id },
      })
      .then((res) => {
        setUser(res.data);
  ...
  return (
    <div className="flex flex-col gap-2.5 p-5 border-b border-gray200">
        ...
        <div className="flex gap-2.5">
          <div className="relative">
            <Image
              ...
              height={50} />
            <div
              className={`absolute top-1 left-1 w-2 h-2 border border-white rounded-full bg-green400
                }`} />
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="text-gray500 text-xs leading-none">
              {getCountryName(proposal?.user.location)}
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2">
        {proposal?.user?.skills &&
          proposal?.user?.skills?.length > 0 &&
          proposal.user.skills.slice(0, 10).map(({
            item,
            index
          }) => (
            <div
              key={`skill-${index}`}
              className="px-2 py-1 text-xs border border-gray-500 rounded leading-none whitespace-nowrap">
              {item}
            </div>
          ))}

        {proposal?.user?.skills && proposal?.user?.skills?.length > 10 && (
          <div
            className="px-2 py-1 text-xs border border-gray500 rounded leading-none whitespace-nowrap">
            {proposal?.user?.skills.length - 10} +
          </div>
        )}
      </div>
      <p className="mt-2 text-gray600 line-clamp-3"> {proposal?.coverLetter}</p>
    </div>
  );
};
