...
const TalentDetails = ({
  ...
  const completedJobs = workHistory.filter((job) => job.status === "completed").length;
  const inProgressJobs = workHistory.filter((job) => job.status === "in_progress").length;
  ...
  return (
    <div
      className="flex flex-col gap-6 pb-4 px-4 overflow-x-visible overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-thumb]:rounded-full">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1">
              <div
                className="flex items-center gap-2.5 text-black text-xl font-medium leading-none">
                <div>{userData.fullName}</div>
                <Icons.verify2 />
              </div>
              <div
                className="flex items-center gap-1 text-gray500 text-sm font-normal leading-none">
                <FaMapLocation />
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-end items-center gap-4">
            <button
              ...
              View profile
              <span
                className="absolute left-0 bottom-0 w-full h-[1px] bg-gradient-to-r from-blue500 to-purple-500"></span>
            </button>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {userData.description && userData.description.length > 200 && (
            <div className="leading-none">
              <button
                className="underline text-green400 hover:text-green500 transition-all leading-none">
                more
              </button>
            </div>
          )}
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-5">
          <div className="flex border-b-2 border-gray200">
            <div
              className="px-2 py-1 -mb-0.5 text-sm text-context border-b-2 border-green500">
              Completed jobs ({completedJobs})
            </div>
            <div
              className="px-2 py-1 -mb-0.5 text-sm text-context border-b-2 border-gray200 hover:border-green500 transition-all">
