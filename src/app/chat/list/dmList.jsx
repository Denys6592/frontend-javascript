...
import { chatSelectUserAtom, DmlistUpdate, userAtom } from "@/store/auth";
...
const DMList = () => {
  ...
  return (
    <div className="relative flex flex-col bg-white border-gray-100 border-r w-80">
      <div
        className="flex flex-col justify-start items-start self-stretch overflow-auto hide-scrollbar">
        <button
          onClick={() => {
            setDrop(!drop);
          }}
          ...
          <div className="relative flex justify-start items-center gap-2.5 h-12">
            ...
            {state === "Online" && (
              <div
                className="absolute bg-emerald-500 mt-10 ml-10 border border-white rounded-full w-2 h-2" />
            )}
            {state === "Idle" && (
              <div
                className="absolute bg-yellow-500 mt-10 ml-10 border border-white rounded-full w-2 h-2" />
            )}
            {state === "Do Not Disturb" && (
              <div
                className="absolute bg-red-500 mt-10 ml-10 border border-white rounded-full w-2 h-2" />
            )}
            {state === "Invisible" && (
              <div
                className="absolute bg-gray-500 mt-10 ml-10 border border-white rounded-full w-2 h-2" />
            )}
          </div>
          <div
            ...
            <div className="inline-flex justify-start items-start self-stretch gap-1">
              <div
                className="flex font-normal text-[#171718] text-sm leading-normal basis-0 grow shrink">
                {state}
              </div>
            </div>
          </div>
      ...
      <div
        ...
        <div
          ...
          <button
            ...
            <div
              className="flex font-normal text-green-500 text-xs leading-none basis-0 grow shrink">
              Online
            </div>
          </button>
          ...
          <button
            ...
            <div
              className="flex font-normal text-yellow-500 text-xs leading-none basis-0 grow shrink">
              Idle
            </div>
          </button>
          <button
      ...
      <div
        className="flex flex-col justify-start items-start self-stretch gap-2.5 py-2.5">
        <button
          onClick={() => {
            setMainBox("friend");
      ...
      <div className="flex justify-between items-center px-5">
        <div
          className="m-2 Sansmr-1 ml-0 w-64 h-5 font-normal text-gray-400 text-base">
          Direct messages
        </div>
      ...
      <div
        className="flex flex-col justify-start items-start self-stretch h-[45vh] overflow-auto hide-scrollbar">
        {user &&
          DM.map((item, index) =>
      ...
      <AddChannelModel isOpen={isChannelModelOpen} onClose={() => setIsChannelModelOpen(false)} />
    </div>
  );
};
