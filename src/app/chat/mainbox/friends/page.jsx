...
const ChatFriend = () => {
  ...
  const handleFriendRequest = () => {
    if (user) {
      const sendMessageRequest = async () => {
        try {
          const status = await addMessageRequest(user.uid, friendEmail);
          if (status) {
            getRequest();
            socket.emit("send-message-request", {
              toUserId: friendEmail,
              fromUserId: user.uid,
            });
          }
        } catch (error) {
  ...
  return (
    <div
      className="inline-flex flex-col justify-start items-end bg-gray-100 flex-1">
      <div
        className="inline-flex justify-start items-center self-stretch gap-3 bg-white p-2.5 border-[#f3f3f3] border-b overflow-hidden">
        <Icons.user />
        <div className="font-semibold text-black text-base leading-normal">
          Message Requests
        </div>
        <div
          className="flex justify-start items-center gap-2.5 h-7 basis-0 grow shrink">
          {/* <button
          ...
          <button
            ...
            <div
              className="font-normal text-gray-400 text-base text-center leading-normal basis-0 grow shrink">
              All
            </div>
          </button>
        ...
        <div
          ...
          <div className="flex flex-col justify-start items-start self-stretch gap-1">
            <div
              className="flex flex-col justify-start items-start self-stretch px-2.5 pb-2">
              <div className="font-normal text-gray-700 text-s text-center leading-3">
                Select Friends
              </div>
            </div>
            <div
              className="inline-flex justify-start items-start self-stretch gap-2.5 px-2.5 py-2">
              <div
                className="font-normal text-[14px] text-gray-700 leading-[8px] basis-0 grow shrink">
            ...
            <FormControlLabel
              ...
              label={
                <div className="relative flex gap-2.5">
                  ...
                  <div
                    className="absolute bg-emerald-500 mt-6 ml-7 border border-white rounded-full w-2 h-2" />
          ...
          <button
            ...
            <div
              className="flex justify-center items-center bg-gradient-to-r from-[#198bd8] to-[#f03dce] px-2.5 py-1.5 rounded-lg h-10 basis-0 grow shrink">
      ...
      <div className="inline-flex justify-start items-start self-stretch">
        {cat === "add" ? (
          <div className="flex-col grow">
            <div
              className="flex-col justify-center items-start gap-3 bg-white p-5 border-[#f3f3f3] border-b h-[152px] overflow-hidden basis-0 shrink">
              ...
              <div
                className="flex flex-col justify-start items-start self-stretch gap-2 bg-white h-[46px]">
                <div
                  className="inline-flex justify-start items-center self-stretch gap-2.5 bg-gray-100 px-2.5 py-2 rounded-lg">
                  <TextField
                    variant="outlined"
                    sx={{
            ...
            <div
              className="flex-col justify-start items-start gap-3 bg-white p-5 border-[#f3f3f3] border-b h-[400px] overflow-hidden">
              <div className="pb-3 font-semibold text-black text-base leading-tight">
                OTHER PLACES TO MAKE FRIENDS
              </div>
              <button
                className="inline-flex justify-start items-center gap-2.5 bg-gray-100 p-2 border border-gray-100 rounded-xl">
                <div
                  className="flex justify-center items-center gap-2.5 bg-emerald-500 rounded-lg w-10 h-10">
                  <Icons.city_1_white />
                </div>
          ...
          <div
            className="inline-flex flex-col justify-start items-start basis-0 grow shrink">
            ...
            {cat === "all" && (
              <div
                className="flex flex-col justify-start items-start self-stretch gap-2 bg-white px-5 py-2.5">
            ...
            {cat === "pending" && (
              <div
                className="flex flex-col justify-start items-start self-stretch gap-2 bg-white px-5 py-2.5">
                <div
                  className="w-[624px] h-2.5 font-medium text-gray-400 text-sm leading-normal">
                  pending
                </div>
                <div className="flex flex-col justify-start items-start self-stretch">
                  {
                    allRequest.map((item, index) => {
                      if (
                        ...
                        return (
                          <FriendItem
                            key={index}
                            // Ensure you have the unique ID for each friend
                            id={item._id}
                            status={item.status}
                            permission={false}
                            avatar={item.recipient.avatar}
