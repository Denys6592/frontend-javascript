...
const FirstStep = ({ user, formik, handleInputChange }) => {
  ...
  return <>
    <div className="flex items-center justify-between">
      <div
        className="w-24 h-24 rounded-full bg-gray100 flex items-center justify-center overflow-hidden border-2 border-gray300 ">
        {image ? (
      ...
      <div className="w-full">
        ...
        <select
          ...
          {locations.map((location) => (
            <option
              className="w-full font-jost text-sm text-black bg-gray100 focus:outline-none"
              key={location.slug}
              value={location.slug}>
              {location.label}
            </option>
