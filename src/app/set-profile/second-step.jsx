...
const SecondStep = ({ user, setUser, formik }) => {
  ...
  const removeWorkHistory = (index) => {
    setUser((prevUser) => {
      const updatedWorkHistory = (prevUser.workHistory ?? []).filter((_, i) => i !== index);
  ...
  const removeEducation = (index) => {
    setUser((prevUser) => {
      const updatedEducation = (prevUser.education ?? []).filter((_, i) => i !== index);
  ...
  const removeSkill = (index) => {
    setUser((prevUser) => {
      const updatedSkills = prevUser.skills.filter((_, i) => i !== index);
      formik.setFieldValue("skills", updatedSkills);
      return { ...prevUser, skills: updatedSkills };
    });
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="w-full pb-5 border-b border-gray200">
        ...
        {user?.workHistory && user?.workHistory.length && user?.workHistory.map((entry, index) => (
          <div key={index} className="flex justify-between mt-3 w-full">
            ...
            <button className="text-red-600 ml-2" onClick={() => removeWorkHistory(index)}>
      ...
      <div className="w-full pb-5 border-b border-gray200">
        ...
        {user?.education && user?.education.length && user?.education.map((entry, index) => (
          <div key={index} className="flex items-center justify-between gap-4 mt-3">
            <div className="w-1/3">
              <Input
            ...
            <button className="text-red-600 ml-2" onClick={() => removeEducation(index)}>
              <RiDeleteBinLine size={20} />
            </button>
          </div>
        ))}
