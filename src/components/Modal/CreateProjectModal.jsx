...
const CreateProjectModal = ({
  ...
  const handleCreateTmProject = () => {
    ...
    if (!tmProject.title || !tmProject.title.trim()) {
      ...
      setError("");
      axios
        .post(`${apiUrl}/api/tmProjects/projects/new`, {
        ...
        .catch((error) => {
          ...
          toast.error(error.response?.data?.message || "Failed to create a new project.");
        });
    }
  };
  ...
  return (
    <div
      className="fixed inset-0 flex justify-center items-center bg-[#17171819] z-50">
      <div
        ref={modalRef}
        ...
        <div className="flex justify-between border-b pb-5 items-center">
          ...
          <div className="relative">
            ...
            <span
              className="absolute h-4 w-4 bg-gray500 rounded-full top-0.5 left-0.5 transition-transform duration-300 peer-checked:bg-white peer-checked:translate-x-4"></span>
          </div>
        </div>
        ...
        <div className="flex flex-col border-b pb-5 gap-5">
          ...
          <div
            className="flex items-center gap-0 rounded-lg overflow-hidden px-0.5 bg-gray100 border border-gray200 focus-within:ring-2 focus-within:ring-green500 transition-all">
            <input
              type="text"
